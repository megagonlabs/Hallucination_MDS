...
def main(input_filepath: str, output_filepath: str, metric_name: str):
    ...
    for resp_id, response in enumerate(data["assignments"]):
        ...
        for preds_info in response["response__parsed__filtered"]:
            pred_uuid = preds_info["pred_uuid"]
            
            if not preds_info["removed"]: # Only consider the predictions that were not removed by filters        
                preds_uuids[pred_uuid].append((preds_info["pred_rank"], preds_info["pred_text"]))
